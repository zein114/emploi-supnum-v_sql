...
<body>
            ...
            <div class="admin-content-inner">
                    ...
                    <div class="glass-card" id="cardProfessors" style="padding: 1.5rem; cursor: pointer;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem;">
                            ...
                            <div style="width: 50px; height: 50px; background: linear-gradient(135deg, rgba(59, 130, 246, 0.1), rgba(99, 102, 241, 0.1)); border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                                <svg style="width: 1.5rem; height: 1.5rem; color: var(--color-primary-blue);" viewBox="0 0 31.979 31.979" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="table-container">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><strong><?= htmlspecialchars($user_row['username']) ?></strong></td>
                                    <td>
                                        <span class="badge badge-<?= $user_row['role'] === 'admin' ? 'warning' : ($user_row['role'] === 'professor' ? 'success' : 'primary') ?>">
                                             <?= $user_row['role'] === 'admin' ? 'Administrateur' : ($user_row['role'] === 'professor' ? 'Professeur' : htmlspecialchars($user_row['role'])) ?>
                                         </span>
                                    </td>
                                    <td><?= date('M d, Y H:i', strtotime($user_row['created_at'])) ?></td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
                        </table>
                ...
                <div class="glass-card" style="padding: 1.5rem;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem;">
                            ...
                            <a href="professors.php" class="btn btn-secondary" style="justify-content: flex-start; padding: 1rem;">
                            ...
                            <a href="timetables.php" class="btn btn-secondary" style="justify-content: flex-start; padding: 1rem;">
                            ...
                            <a href="weekly_workload.php" class="btn btn-secondary" style="justify-content: flex-start; padding: 1rem;">
                                <svg style="width: 1.25rem; height: 1.25rem; margin-right: 0.75rem;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (professorsCard && window.Modal && typeof Modal.showContent === 'function') {
                professorsCard.addEventListener('click', function () {
                    var html = '';
                    if (professorsData && professorsData.length) {
                        html += '<div class="table-container">';
                        html += '<table class="table">';
                        html += '<thead><tr><th>Nom</th><th>Email</th><th>Créé le</th></tr></thead><tbody>';
                        professorsData.forEach(function (prof) {
                            var createdAt = prof.created_at ? new Date(prof.created_at).toLocaleString('fr-FR') : '';
                            html += '<tr>' +
                        ...
                        html += '<div class="text-muted" style="padding: 1rem; text-align: center;">Aucun professeur trouvé.</div>';
                    }
                    html += '<div style="margin-top: 1.5rem; display: flex; justify-content: flex-end;">' +
                        '<a href="users.php" class="btn btn-secondary" style="margin-top: -0.5rem; margin-bottom: -1rem;">Voir plus</a>' +
                        '</div>';

                    Modal.showContent('Derniers professeurs ajoutés', html);
                });
            }

            var groupsCard = document.getElementById('cardGroups');
            if (groupsCard && window.Modal && typeof Modal.showContent === 'function') {
                groupsCard.addEventListener('click', function () {
                    var html = '';
                    if (groupsData && groupsData.length) {
                        var grouped = {};
                        groupsData.forEach(function (item) {
                            // Use 'semester' instead of 'year_name'
                            var sem = item.semester || 'Sans semestre';
                            if (!grouped[sem]) {
                                grouped[sem] = [];
                            }
                            grouped[sem].push(item.name || '');
                        });

                        // Sort semantics if possible (e.g. S1, S2...)
                        var keys = Object.keys(grouped).sort();

                        keys.forEach(function (semName) {
                            html += '<div style="margin-bottom: 1rem;">';
                            html += '<div style="font-weight: 600; margin-bottom: 0.25rem; color: var(--color-primary-blue);">' + semName + '</div>';
                            html += '<div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">';
                            grouped[semName].forEach(function (groupName) {
                                html += '<span class="badge badge-primary">' + groupName + '</span>';
                            });
                            html += '</div>';
                            html += '</div>';
                        });
                    } else {
                        html += '<div class="text-muted" style="padding: 1rem; text-align: center;">Aucun groupe trouvé.</div>';
                    }
            ...
            if (roomsCard && window.Modal && typeof Modal.showContent === 'function') {
                roomsCard.addEventListener('click', function () {
                    ...
                    if (roomsData && roomsData.length) {
                        ...
                        Object.keys(groupedRooms).sort().forEach(function (type) {
                            html += '<div style="margin-bottom: 1rem;">';
                            html += '<div style="font-weight: 600; margin-bottom: 0.25rem; color: var(--color-warning);">' + type + '</div>';
                            html += '<div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">';
                            groupedRooms[type].forEach(function (room) {
                                var title = room.name;
                                if (room.capacity) title += ' (' + room.capacity + ' pl.)';
                                html += '<span class="badge badge-warning">' + title + '</span>';
                            });
                            html += '</div>';
                            html += '</div>';
                        });
                    } else {
                        html += '<div class="text-muted" style="padding: 1rem; text-align: center;">Aucune salle trouvée.</div>';
                    }
                    Modal.showContent("Salles disponibles par Type", html);
                });
            }
        });
    </script>
